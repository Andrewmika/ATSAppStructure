...
static NSString *const kDBModule = @"DBStructure";
#define kDBQueueSpecific "com.andrewshen.structureDBQueue"
...
@property (nonatomic, strong, readwrite)  id<TemplateProtocol>  templateDAO; // 患者列表
...
- (void)closeDB {
    if (self.DBQueue) {
        [self.DBQueue close];
        self.uid = nil;
        self.templateDAO = nil;
    }
}

- (void)p_createTables {
    [self.DBQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        [TemplateDAOImpl createMessageTableWithDB:db];
    }];
}
...
- (id<TemplateProtocol>)templateDAO {
    if (!_templateDAO) {
        _templateDAO = [[TemplateDAOImpl alloc] initWithDBQueue:self.DBQueue DBActionQueue:self.DBActionQueue];
    }
    return _templateDAO;
}
